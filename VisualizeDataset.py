...
class VisualizeDataset:
    ...
    def plot_dataset(self, data_table, columns, match='like', display='line'):
        ...
        if len(columns) > 1:
            f, xar = plt.subplots(len(columns),figsize=(10, 8), sharex=True, sharey=False)
        else:
            f, xar = plt.subplots()
            xar = [xar]
        ...
        for i in range(0, len(columns)):
            ...
            xar[i].legend(relevant_cols, fontsize='small', numpoints=1, loc='lower left',ncol=len(relevant_cols), fancybox=True, shadow=True)
